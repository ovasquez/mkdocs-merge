...
def cli():
    ...
    This simple tool allows you to merge the sources of multiple MkDocs sites into a single one
    converting each of the specified sites to a subpage of the master site.
    """
...
def run(master_site, sites):
    """
    Merges into MASTER_SITE the SITES.

    MASTER_SITE: Path to the base site in which all other sites will be merged into.
    The mkdocs.yml file of this site will be preserved as is, except for the new pages.

    SITES: Paths to the sites to be merged. Each of this sites will be converted to a
    subpage of the master site. Their mkdocs.yml files will be ignored except for the
    pages data.
    """

    # Custom argument validation instead of an ugly generic error
    if not sites:
        click.echo('Please specify one or more sites to merge to the master site.\n'
    ...
    if not os.path.isfile(master_yaml):
        click.echo('Could not find the master site yml file, make sure it exists: '
                   + master_yaml)
        return
...
def merge_sites(sites, master_site):
    """
    Adds the sites' "pages" section to the master_data and copies the sites' content
    to the master_site.
    """
    new_pages = []
    for site in sites:
        ...
        if not os.path.isfile(site_yaml):
            click.echo('Could not find the site yml file, this site will be skipped: '
                       + site_yaml)
            continue
        ...
        try:
            ...
            click.echo('Could not find the site_name in the yml file, defaulting to the folder name'
                       + site_name)

        site_root = site_name.replace(' ', '_').lower()

        # Copy site's files to a new directory in the master site's "docs" directory
        ...
        if not os.path.isdir(old_site_docs):
            click.echo('Could not find the site "docs" folder, this site will be skipped: '
                       + old_site_docs)
            continue
        ...
        # Update the 'pages' data with the new path only after files have been copied
...
def update_pages(pages, site_root):
    """
    Recursively traverses the lists of pages (dictionaries) to update the path of the pages
    with the site_name, used as a subsection in the merged site.
    """
